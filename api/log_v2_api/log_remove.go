...
import (
	...
	log_stash "go-vue-blog-study/plugins/log_stash_v2"
	"time"
)
...
func (LogApi) LogRemoveView(c *gin.Context) {
	...
	if len(cr.IDList) > 0 {
		...
		if startTimeErr != nil {
			res.FailWithMessage("开始时间格式错误", c)
			return
		}
		if endTimeErr != nil {
			res.FailWithMessage("结束时间格式错误", c)
			return
		}
	...
	res.OkWithMessage(fmt.Sprintf("共删除 %d 条日志", len(logs)), c)
}
